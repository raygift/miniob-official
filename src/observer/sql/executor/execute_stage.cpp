...
void print_aggre_header(std::ostream &os, const AggregateOperator &oper)
{
  const int cell_num = oper.tuple_cell_num();
  const TupleCellSpec *cell_spec = nullptr;
  for (int i = 0; i < cell_num; i++) {
...
RC do_multi_select(SQLStageEvent *sql_event)
{
  SelectStmt *select_stmt = (SelectStmt *)(sql_event->stmt());
  SessionEvent *session_event = sql_event->session_event();
  RC rc = RC::SUCCESS;
  if (select_stmt->tables().size() != 1) {
    LOG_WARN("lets try to select more than 1 tables !!!");
  }

  CartesianOperator *cart_oper = new CartesianOperator(select_stmt);

  PredicateOperator pred_oper(select_stmt->filter_stmt());
  pred_oper.add_child(cart_oper);
...
RC do_statistic(SQLStageEvent *sql_event)
{
  SelectStmt *select_stmt = (SelectStmt *)(sql_event->stmt());
  SessionEvent *session_event = sql_event->session_event();
  RC rc = RC::SUCCESS;
...
RC ExecuteStage::do_select(SQLStageEvent *sql_event)
{
  ...
  if (select_stmt->tables().size() != 1) {
    ...
    // rc = RC::UNIMPLENMENT;
    rc = do_multi_select(sql_event);
    return rc;
  }

  if (select_stmt->have_aggregation()) {
    rc = do_statistic(sql_event);
    return rc;
  }
...
RC ExecuteStage::do_create_index(SQLStageEvent *sql_event)
{
  ...
  if (create_index.attribute_count <= 1) {
    ...
    RC rc = table->create_multi_index(
        nullptr, create_index.index_name, create_index.attributes, create_index.attribute_count, create_index.unique);
    sql_event->session_event()->set_response(rc == RC::SUCCESS ? "SUCCESS\n" : "FAILURE\n");
    return rc;
  }
}
